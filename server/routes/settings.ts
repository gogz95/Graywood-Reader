...
settingsRouter.get("/api/settings/backup/export", (_req, res) => {
  const backup = {
    ...
    readingProgress: SqliteDb.getAllReadingProgressForUser('usr_admin'),
    userLibraryStates: SqliteDb.getAllUserLibraryStates('usr_admin'),
    config: syncConfig,
...
settingsRouter.post("/api/settings/backup/import", (req, res) => {
  try {
    const {
      mangaDatabase: importedManga,
      config: importedConfig,
      appSettings: importedSettings,
      readingProgress: importedProgress,
      userLibraryStates: importedStates,
    } = req.body;
