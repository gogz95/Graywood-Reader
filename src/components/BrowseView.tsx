...
import { MangaItem, MangaType, ReadingStatus, hasWorkingReaderSource, isMangaDexSourceLink } from '../types';
import {
  Compass,
  Filter,
  ArrowUpDown,
...
// (source links, alt titles, genres, progress) instead of dropping the losing entry.
function mergeMangaItems(a: MangaItem, b: MangaItem): MangaItem {
...
// are NOT merged.
function dedupeCatalog(list: MangaItem[]): MangaItem[] {
...
export const BrowseView: React.FC<BrowseViewProps> = ({
  ...
  const filteredManga = useMemo(() => {
    ...
    // 2. Deduplicate by apiId then normalized title so the same series never appears
    //    multiple times. Duplicate entries are MERGED (not dropped) so all sources,
    //    alt-titles, and genres are preserved.
