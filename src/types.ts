...
export function getNsfwDetectionReason(manga?: {
  genres?: string[];
  title?: string;
  altTitles?: string[];
  notes?: string;
  ...
  // 1. Source Origin & Domain Checks
  const knownNsfwSources = ['manhwa18', 'adult webtoon', 'adultwebtoon', 'hiperdex', 'beehentai', 'hentai20', 'hotcomics', 'daycomics', 'nhentai', 'hitomi', 'pururin', 'e-hentai', 'exhentai'];
  ...
  for (const src of sourcesToCheck) {
    ...
    for (const ns of knownNsfwSources) {
      if (sName.includes(ns)) {
        return `Imported from 18+ adult source (${src.sourceName || ns})`;
      }
      if (sUrl && sUrl.includes(ns)) {
        return `Live URL matches known adult domain (${ns})`;
      }
    }
  }
  ...
  for (const g of genres) {
    ...
    if (
      ...
      return `Contains 18+ adult genre tag "${g}"`;
    }
  }
  ...
  if (
    titleText.includes('[18+]') ||
    titleText.includes('(18+)') ||
    titleText.includes(' 18+ ') ||
    titleText.includes('[nsfw]') ||
    titleText.includes('(nsfw)') ||
    titleText.includes('[uncensored]') ||
    titleText.includes('(uncensored)') ||
    titleText.includes('[smut]') ||
    titleText.includes('[hentai]') ||
    titleText.includes('[adult]') ||
    titleText.includes('[r18]') ||
    titleText.includes('[r-18]') ||
    titleText.includes('[raw 18+]') ||
    titleText.includes('uncensored ver') ||
    titleText.includes('18+ ver')
  ) {
    return 'Title contains 18+ / adult classification keyword';
  }

  // 4. Description Disclaimer Checks
  const descText = (manga.description || '').toLowerCase();
  if (
    descText.includes('18+ only') ||
    descText.includes('explicit adult content') ||
    descText.includes('sexually explicit') ||
    descText.includes('for adult readers') ||
    descText.includes('mature audiences only') ||
    descText.includes('erotic webtoon') ||
    descText.includes('uncensored chapters') ||
    descText.includes('smut version')
  ) {
    return 'Description contains explicit adult content disclaimer';
  }
